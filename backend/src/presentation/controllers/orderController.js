...
async function createOrder(req, res) {
  try {
    ...
    if (!number_id || !provider_id || !operator_id) {
      return res
        .status(400)
        .json({ success: false, message: 'number_id, provider_id, operator_id wajib diisi' });
    }
...
async function checkOrder(req, res) {
  try {
    const order = await usecases.checkOrderStatus.execute(req.params.orderId);
    res.json({ success: true, data: order });
  } catch (err) {
    res.status(err.status || 500).json({ success: false, message: err.message });
  }
...
async function setOrderStatus(req, res) {
  try {
    ...
    const order = await usecases.setOrderStatus.execute(req.params.orderId, status);
    res.json({ success: true, data: order });
  } catch (err) {
    res.status(err.status || 500).json({ success: false, message: err.message });
  }
...
async function listHistory(req, res) {
  try {
    const orders = await usecases.listOrderHistory.execute();
